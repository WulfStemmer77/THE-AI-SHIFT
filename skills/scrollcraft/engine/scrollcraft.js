/* ============================================================================
   ...
   ACTS: data-sc-act="scrub|pin|pan|flow", data-sc-span, data-sc-dwell.
   DEVICES: scrub, sequence, pan, parallax, cue, kinetic, reveal, count, drift.
   WORLDFLIGHT: one fixed stage, weighted segments, crossfade seams, copy windows.
   POINTER: tilt, magnet and spotlight. All disabled under reduced motion/touch.
   ========================================================================== */
...
(function (global) {
  ...
  function mount(root, opts) {
    ...
    function layout() {
      ...
      acts.forEach(function (a) {
        ...
        if (pos !== 'sticky' && pos !== '-webkit-sticky') {
          console.warn('[scrollcraft] act "' + (a.el.id || a.device) + '" will not pin: stage position is ' + pos + ', not sticky.', a.stage);
        }
      });
      worlds.forEach(function (W) {
        ...
        W.checked = true;
        if (!W.stage) {
          console.warn('[scrollcraft] worldflight has no [data-sc-world] stage.', W.el);
          return;
        }
        if (!W.spacer) console.warn('[scrollcraft] worldflight has no [data-sc-spacer].', W.el);
        var wp = getComputedStyle(W.stage).position;
        if (wp !== 'fixed') console.warn('[scrollcraft] worldflight stage is not fixed.', W.stage);
      });
      read();
    }
    ...
    function read() {
      ...
      for (var i = 0; i < acts.length; i++) {
        ...
        for (var rv = 0; rv < a.reveals.length; rv++) {
          var R = a.reveals[rv];
          var rt = smooth((a.p - R.from) / Math.max(R.to - R.from, 0.001));
          var pct = ((1 - rt) * 100).toFixed(2);
          R.el.style.clipPath =
            R.dir === 'down' ? 'inset(' + pct + '% 0 0 0)' :
            R.dir === 'left' ? 'inset(0 ' + pct + '% 0 0)' :
            R.dir === 'right' ? 'inset(0 0 0 ' + pct + '%)' :
            R.dir === 'iris' ? 'circle(' + (rt * 78).toFixed(2) + '% at 50% 50%)' :
                               'inset(0 0 ' + pct + '% 0)';
        }
      ...
      if (driftA) docEl.style.setProperty('--sc-canvas', mixColor(driftB.rgb, driftA.rgb, driftT));
    ...
    function initPointer() {
      ...
      Array.prototype.forEach.call(root.querySelectorAll('[data-sc-spotlight]'), function (el) { spots.push(el); });
